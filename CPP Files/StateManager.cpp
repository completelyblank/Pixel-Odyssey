...
void StateManager::handleInput(sf::RenderWindow& window, sf::View& view, Clock& track) {
    if (!states.empty()) {
        states.top()->handleInput(window, view, track);
    }
}
...
void StateManager::render(RenderWindow& window, Clock& track) {
    if (!states.empty()) {
        states.top()->render(window, track);
    }
}
