...
void Bounds::set(BeachCoordinates& cord) {
    bound.clear();
    for (int i = 0; i < cord.coordinates.size(); i++) {
        rec.setPosition(cord.coordinates[i]);
        bound.push_back(rec);
...
void Bounds::set(Maze& cord) {
    bound.clear();
    for (int i = 0; i < cord.coordinates.size(); i++) {
        rec.setSize(Vector2f(25.f, 20.f));
...
void Bounds::set(ParkCoordinates& cord) {
    bound.clear();
    for (int i = 0; i < cord.coordinates.size(); i++) {
        rec.setSize(Vector2f(30.f, 40.f));
        rec.setPosition(cord.coordinates[i]);
        bound.push_back(rec);
    }
}

void Bounds::set(CityCoordinates& cord) {
    bound.clear();
    for (int i = 0; i < cord.coordinates.size(); i++) {
        rec.setPosition(cord.coordinates[i]);
        bound.push_back(rec);
...
Vector2f Bounds::collision(Sprite& sp, float velocityX, float velocityY) {
...
Vector2f Bounds::collision(Sprite& sp, Vector2f velocity, vector <Sprite> sprites)
{
    for (auto& i : sprites)
    {
        FloatRect spritebounds = sp.getGlobalBounds();
